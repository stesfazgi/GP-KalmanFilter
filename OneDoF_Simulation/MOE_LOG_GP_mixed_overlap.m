classdef MOE_LOG_GP_mixed_overlap <handle
...
%                     end
            ...
            if lcount == obj.pts && rcount == 0  %if left set has all the points now
                div = 1; % output to keep dividing
                childModel = (obj.count);
            elseif rcount == obj.pts && lcount == 0 %if right set has all the points now
...
%                 B = (1:obj.pts);
                ...
                obj.K(1:lcount,(obj.auxUbic(obj.count)-1)*obj.pts+1:...
                    (obj.auxUbic(obj.count)-1)*obj.pts+lcount) = newKl;
                obj.K(1:rcount,(obj.auxUbic(obj.count+1)-1)*obj.pts+1:...
                    (obj.auxUbic(obj.count+1)-1)*obj.pts+rcount) = newKr;
...
%             obj.count
            while true
                if obj.children(1,model) ~=-1
                  [pL, ~] = obj.activation(x, model);
                  
                  if pL == 1
                    model = obj.children(1,model);%left child
                  elseif pL > 0 && pL < 1 % overlap
                    modelstore = [modelstore obj.children(2,model)];%right child
                    model = obj.children(1,model);%left child
                  else
                    model = obj.children(2,model);
                  end
                  
                else %if model is a child
                  obj.addPoint(x,y,model)
                  storeid = storeid+1;
                  if storeid > length(modelstore)
                    break
                  else
                    model = modelstore(storeid);
                  end
                end
            end  
        end
        ...
        function [out, outVar,outderiv, outModels] = predict(obj,x)
            ...
            for i=1:mCount
                ...
                xstardiff = ((loctraindata - x).').*((obj.sigmaL).');
