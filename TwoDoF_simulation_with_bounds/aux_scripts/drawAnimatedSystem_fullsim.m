function [fig_simulation,video_handle] = drawAnimatedSystem_fullsim(t,x,x_ref,l,tau, tau_d,tau_d_KF_hat_plot,AKF_lbs,AKF_ubs,gptime_plot,tau_d_nolearn)
%
...
%     widthupper = 0.1;
    ...
    while i + increment < length(t)   
        ...
        if i <= 15000
          phase = 1;
        elseif i > 15000 && i <= 30000
          phase = 2;
        elseif i > 30000 && i <= 45000
          phase = 3;
        else
          phase = 4;
        end
...
%         title('Planar robot with 2 degrees of freedom');
        ...
        if phase==1
          txt4 = text(0.35, 0.95, "LoG-GP update time: " + sprintf('%.1e',gptime_plot(i)) + "s", 'Units', 'normalized');
        else
          txt4 = text(0.35, 0.95, "LoG-GP predict time: " + sprintf('%.1e',gptime_plot(i)) + "s", 'Units', 'normalized');
        end
...
%         set(findall(gcf,'-property','FontSize'),'FontSize',18)
        ...
        xt = t((phase-1)*15000+1:increment:i);
        x2 = [xt', fliplr(xt')];
        conf_region1 = [AKF_lbs((phase-1)*15000+1:increment:i,9)', fliplr(AKF_ubs((phase-1)*15000+1:increment:i,9)')];
        conf_region2 = [AKF_ubs((phase-1)*15000+1:increment:i,10)', fliplr(AKF_lbs((phase-1)*15000+1:increment:i,10)')];
        
        subplot(2,5,[4, 5])
        title('Link1 external torque estimation');
        
        fill(x2, conf_region1,[0, 1, 1],'EdgeColor','none'); hold on;
        plot(xt,tau_d_KF_hat_plot((phase-1)*15000+1:increment:i,1),'b','LineWidth',3);hold on;
        if phase == 2
          plot(xt,tau_d_nolearn((phase-1)*15000+1:increment:i,1),'m','LineWidth',2.5);hold on;
        end
        plot(xt,tau_d((phase-1)*15000+1:increment:i,1),'--','Color',tum_orange,'LineWidth',3); hold on;
        plot(xt,tau((phase-1)*15000+1:increment:i,1),'Color',human_joint_color,'LineWidth',2); hold on;
        
        

        grid on;
        ylim([-10,65]);
        xlim([t((phase-1)*15000+1),t(phase*15000)]);
        ylabel("Torque [Nm]"); xlabel("Time [s]");
%         set(findall(gcf,'-property','FontSize'),'FontSize',18)
        if phase == 2
          legend1 = legend('$2\sigma$ confidence region','Estimate $\hat{\tau}_{ext,1}$','Estimate \textbackslash wo GP $\hat{\tau}_{ext,1}$','Ground truth $\tau_{ext,1}$','Motor torque $\tau_{m,1}$','Interpreter', 'latex', 'Location', 'north','FontSize',14);
        else
          legend1 = legend('$2\sigma$ confidence region','Estimate $\hat{\tau}_{ext,1}$','Ground truth $\tau_{ext,1}$','Motor torque $\tau_{m,1}$','Interpreter', 'latex', 'Location', 'north','FontSize',14);
        end
        subplot(2,5,[9, 10])
        title('Link2 external torque estimation');
        
        fill(x2, conf_region2,[0, 1, 1],'EdgeColor','none'); hold on;
        plot(xt,tau_d_KF_hat_plot((phase-1)*15000+1:increment:i,2),'b','LineWidth',3);hold on;
        if phase == 2
          plot(xt,tau_d_nolearn((phase-1)*15000+1:increment:i,2),'m','LineWidth',2.5);hold on;
        end
        plot(xt,tau_d((phase-1)*15000+1:increment:i,2),'--','Color',tum_orange,'LineWidth',3); hold on;
        plot(xt,tau((phase-1)*15000+1:increment:i,2),'Color',human_joint_color,'LineWidth',2); hold on;
%         legend('AKF estimate $\hat{\tau}_{ext,2}$','$2\sigma$ ucb augmented state KF','$2\sigma$ lcb augmented state KF','true external torque $\tau_{ext,2}$','Interpreter', 'latex');
        ...
        set(findall(gcf,'-property','FontSize'),'FontSize',18)
        if phase == 2
          legend('$2\sigma$ confidence region','Estimate $\hat{\tau}_{ext,2}$','Estimate \textbackslash wo GP $\hat{\tau}_{ext,2}$','Ground truth $\tau_{ext,2}$','Motor torque $\tau_{m,2}$','Interpreter', 'latex', 'Location', 'north','FontSize',14);
        else
          legend('$2\sigma$ confidence region','Estimate $\hat{\tau}_{ext,2}$','Ground truth $\tau_{ext,2}$','Motor torque $\tau_{m,2}$','Interpreter', 'latex', 'Location', 'north','FontSize',14);
        end
        legend1.FontSize = 14;
        frame = getframe(gcf);
