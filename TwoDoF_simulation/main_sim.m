...
P_KF = blkdiag(0.01*eye(AKF.nom_model.dofs*4),eye(AKF.nom_model.distorder*AKF.nom_model.dofs));
...
x_KF_hat = [current_x(3); current_x(7); current_x(1)-current_x(3)/AKF.nom_model.Nm(1); current_x(5)-current_x(7)/AKF.nom_model.Nm(2);...
    ...
    0; zeros(AKF.nom_model.distorder-1,1);0;zeros(AKF.nom_model.distorder-1,1)];
...
for i=1:(length(t_span)-1)
    ...
    tau_m = ctc_controller_robot(AKF.nom_model, current_x, theta_second_deriv, M, n, x_ref, i, sim_phase);
    if sim_phase >= 2
    ...
    if i>1
        ...
        if sim_phase == 1
            ...
            for k = ids
                id = id+1;
                plots.AKF_ubs(i,id) = x_KF_hat(k) + P_KF_bounds(k,k);
                plots.AKF_lbs(i,id) = x_KF_hat(k) - P_KF_bounds(k,k);
